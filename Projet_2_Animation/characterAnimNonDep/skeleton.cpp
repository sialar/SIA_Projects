...
glm::vec3 Skeleton::distance(std::vector<glm::vec3>& vector)
{
...
void Skeleton::computeAxisAngles()
{
	double x = 0, y = 0, z = 0;
	qglviewer::Vec vaa;
	for (int i = 0; i < _dofs[0]._values.size(); i++)
	{
...
glm::vec3 Skeleton::rotationIsConstant(double threshold)
{
	...
	for (int i = 0; i < axisAngles.size(); i++)
	{
		j = 0;
		temp = axisAngles[j].x;
		while (j < axisAngles.size() && abs(temp-axisAngles[j].x)<=threshold)
			j++;
		if (j == axisAngles.size())
			boolRes.x = 1;
		
		j = 0;
		temp = axisAngles[j].y;
		while (j < axisAngles.size() && abs(temp - axisAngles[j].y)<=threshold)
			j++;
		if (j == axisAngles.size())
			boolRes.y = 1;
		...
		j = 0;
		temp = axisAngles[j].z;
		while (j < axisAngles.size() && abs(temp - axisAngles[j].z)<=threshold)
			j++;
		if (j == axisAngles.size())
			boolRes.z = 1;
...
void Skeleton::nbDofs() {
	...
	computeAxisAngles();
	glm::vec3 constRotations = rotationIsConstant(0.001);
	// To test max distance between rotaion angles for each axe
	//glm::vec3 dist = distance(axisAngles);
	//cout << _name << endl << dist.x << " " << dist.y << " " << dist.z << endl << endl;
