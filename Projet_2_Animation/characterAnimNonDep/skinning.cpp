...
void Skinning::computeWeights() {
	...
	{
		int min_index = 0;
		float min_dist = glm::length(_posBonesInit[0]-_skin->_points[i]);
		for (int j = 0; j < _nbJoints; j++)
		{
			_weights[i][j] = 0;
			if (glm::length(_posBonesInit[j]-_skin->_points[i]) < min_dist)
			{
				min_index = j;
				min_dist = glm::distance(_posBonesInit[j], _skin->_points[i]);
			}
		}
		_weights[i][min_index] = 1;
	}
	// Test
	// -----> Ordre de grandeur des pos des vertex et des pos des bones différent
	// -----> Probleme scaling
	/*
	for (int i = 0; i < 1000; i++)
	{
		for (int j = 0; j < 1; j++)
			cout << _skin->_points[i].x << " " << _skin->_points[i].y << " " << _skin->_points[i].z << " \n";
		cout << endl;
	}
	*/ /*
	for (int i=0; i<_posBonesInit.size(); i++)
		cout << i << " --> " << _joints[i]->_name << ": " << _posBonesInit[i].x << ", " << _posBonesInit[i].y << ", " << _posBonesInit[i].z << endl << endl;
	*/
...
void Skinning::paintWeights(std::string jointName) {
	...
	for (int i = 0; i < _nbVtx; i++) 
		if (_weights[i][jointIndex] == 1)
			nb++;
	cout << "Number of \"weight==1\" = " << nb << endl;
	*/
}
...
void Skinning::animate() {
	...
	// init skinning doit etre fait avant le commencement de l'animation du skeleton.
	// car les positions initiales du skeleton (_posBonesInit) ne sont pas définis avant le lancement de l'animation:
	// glGetFloatv(GL_MODELVIEW_MATRIX, ptr); donne "nan"
	if (!_initDone) 	init();
